...
function Dashboard() {
  ...
  async function fetchDashboardMetrics() {
    try {
      setLoading(false);
      const { data: studentsData } = await supabase.from("students").select("*").order("id", { ascending: false });
      const { data: teachersData } = await supabase.from("teachers").select("id");
      const { data: paymentsData } = await supabase.from("payments").select("*").order("id", { ascending: false });

      const totalStuds = studentsData ? studentsData.length : 0;
      const totalTechs = teachersData ? teachersData.length : 0;
      const revenue = paymentsData ? paymentsData.reduce((sum, p) => sum + Number(p.amount || 0), 0) : 0;
      const dues = paymentsData ? paymentsData.reduce((sum, p) => sum + Number(p.dues || 0), 0) : 0;

      setStats({ totalStudents: totalStuds, totalTeachers: totalTechs, totalCollected: revenue, totalOutstanding: dues });
    ...
    <>
      ...
      <div className="cards" style={{ marginBottom: "35px" }}>
        <div className="card" style={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          ...
          <p>{stats.totalStudents} <span style={{ fontSize: "14px", fontWeight: "400", color: "var(--muted)" }}>Students</span></p>
        </div>
        ...
        <div className="card" style={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          ...
          <p>{stats.totalTeachers} <span style={{ fontSize: "14px", fontWeight: "400", color: "var(--muted)" }}>Teachers</span></p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px", marginBottom: "40px" }}>
        ...
        <button className="theme-btn" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "10px", padding: "16px", marginTop: 0 }} onClick={() => navigate("/fees")}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(480px, 1fr))", gap: "24px", marginBottom: "40px" }}>
        <div>
          <h4 style={{ color: "#ffffff", fontSize: "15px", marginBottom: "12px", fontWeight: "600", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span>Recently Enrolled Candidates</span>
            <span style={{ fontSize: "12px", color: "var(--accent)", cursor: "pointer" }} onClick={() => navigate("/students")}>View Database →</span>
          </h4>
                ...
                {recentStudents.length === 0 ? (
                  ...
                  recentStudents.map((student) => (
                    <tr key={student.id} onClick={() => navigate(`/students/${student.id}`)}>
                      <td style={{ color: "#a5b4fc", fontWeight: "600" }}>{student.student_id}</td>
                      <td>{student.name}</td>
                      <td>{student.class}</td>
                      <td>
        ...
        <div>
          <h4 style={{ color: "#ffffff", fontSize: "15px", marginBottom: "12px", fontWeight: "600", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: "12px", color: "var(--accent)", cursor: "pointer" }} onClick={() => navigate("/fees")}>View Ledger →</span>
          </h4>
          <div className="table-container">
            <table>
              <thead>
              ...
              <tbody>
                {recentPayments.length === 0 ? (
                  <tr><td colSpan="4" style={{ textAlign: "center", color: "var(--muted)", padding: "20px" }}>No recent payments.</td></tr>
                ) : (
                  recentPayments.map((payment) => (
                    <tr key={payment.id}>
                      <td style={{ color: "var(--muted)" }}>{payment.receipt_no}</td>
